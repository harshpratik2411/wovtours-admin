...
const Destination = () => {
 ...
 const [DestinationList, setDestinationList] = useState([]);
  ...
  DestinationServices.getAll(search, order, page, currentStatus)
      .then((response) => {
      

     setDestinationList(response.Destinations);
      ...
      <tr key={destination.id} className="border-b hover:bg-gray-50">
        <td className="py-4">
          {destination.media_url ? (
            destination.media_url.match(/\.(jpeg|jpg|gif|png|webp)$/i) ? (
              <img
          ...
          <td className="py-6">
            <p className="font-medium text-gray-800">{destination.title}</p>
          </td>
          <td className="py-6">
            <p className="font-medium ml-3  text-gray-800">{destination.description}</p>
          </td>
         <td className="py-6">
            ...
            <div className="relative inline-block">
              ...
              {activeMenu === destination.id && (
                <div className="dropdown-menu absolute right-0 -top-[4rem] z-10 bg-white border rounded shadow w-32">
                  ...
                  <button
                    onClick={() =>
                      navigate(`/destinations/update/${destination.id}`)
                    }
                    className="flex items-center gap-2 px-3 py-2 w-full hover:bg-gray-100 text-sm text-gray-700"
                  >
...
<div className="block sm:hidden space-y-5 flex-col items-center justify-center">
  {!loading && DestinationList && DestinationList.length > 0 ? (
    DestinationList.map(( 
      destination  
    ) => ( 
      <div 
