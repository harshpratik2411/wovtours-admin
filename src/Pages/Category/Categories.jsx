...
const Categories = () => {
 ...
 const [categoryList, setCategoryList] = useState([]);
  ...
  CategoryServices.getAll(search, order, page, currentStatus)
      .then((response) => {
  ...
  <tbody>
    ...
    categoryList.map((category) => (
      <tr key={category.id} className="border-b hover:bg-gray-50">
        <td className="py-4">
          {category.media_url ? (
            category.media_url.match(/\.(jpeg|jpg|gif|png|webp)$/i) ? (
              <img
...
{activeMenu === category.id && (
  <div className="mt-3 dropdown-menu bg-white border rounded shadow w-full z-10">
    <button
      onClick={() => navigate(`/categories/view/${category.slug}`)}
      className="flex items-center gap-2 px-3 py-2 w-full hover:bg-gray-100 text-sm text-gray-700"
    >
      <FaEye size={14} /> View
    </button>
    <button
      onClick={() => navigate(`/categories/update/${category.slug}`)}
      className="flex items-center gap-2 px-3 py-2 w-full hover:bg-gray-100 text-sm text-gray-700"
    >
      <FaEdit size={14} /> Update
    </button>
