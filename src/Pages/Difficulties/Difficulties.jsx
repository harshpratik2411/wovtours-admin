...
const Difficulties = () => {
  ...
  const updateBooked = (id, delta) => {
    setDifficultyList((prev) =>
      prev.map((difficulty) =>
        difficulty.id === id
          ? { ...difficulty, booked: Math.max(0, difficulty.booked + delta) }
          : difficulty
      )
    );
  };
              ...
              {displayed.map((difficulty) => (
                ...
                >
                  <td className="py-2 px-2  sm:px-0">
                    <p className="font-medium lg:ml-0 ml-[2px] text-gray-800">
                      {difficulty.name}
                    </p>
                  </td>
                  ...
                  <td className="py-2 px-2 sm:px-0">
                    <span
                      className={`px-3 py-1  rounded-full text-left text-xs font-medium ${getStatusClass(
                        difficulty.status
                      )}`}
                    >
                      {difficulty.status}
                  ...
                  <td className="py-2 px-2 lg:ml-0 ml-[2px] sm:px-0 text-gray-700">
                    {difficulty.slug}
                  </td>

                  <td className="py-2 px-2 sm:px-0 text-gray-700">
                    <span className="lg:ml-20 font-semibold">
                    ...
                    <div className="relative inline-block">
                      ...
                      {activeMenu === difficulty.id && (
                        <div className="dropdown-menu absolute right-0 -top-20 z-10 bg-white border rounded shadow w-32">
                          <button
                            onClick={() =>
                              navigate(`/difficulties/view/${difficulty.id}`)
                            }
                          ...
                          <button
                            onClick={() =>
                              navigate(`/difficulties/update/${difficulty.id}`)
                            }
                            className="flex items-center gap-2 px-3 py-2 w-full hover:bg-gray-100 text-sm text-gray-700"
                          >
