...
const FeaturedCategory = () => {
    ...
    const [FeaturedCategoryList, setFeaturedCategoryList] = useState([]);
    ...
    function apiCall(
        ...
        FeaturedCategoryServices.getAll(search, order, page, currentStatus)
            .then((response) => {
                setFeaturedCategoryList(response.featuredCategories);
                setTotalPages(response.totalPages);
                setCurrentPage(response.currentPage);
                setLoading(false);
            })
            .catch((err) => {
                setFeaturedCategoryList([]);
                setTotalPages(1);
                setLoading(false);
            });
    }
        ...
        <>
            ...
            <div className="bg-white  p-4 sm:p-6 lg:ml-72 rounded-xl shadow-md font-rubik w-full max-w-6xl mx-auto -mt-2 mb-12">
                ...
                <div className="flex flex-col sm:flex-row sm:justify-between items-start sm:items-center gap-3 mb-4">
                    ...
                    {/* Add Button */}
                    <button
                        onClick={() => navigate("/featured-categories/add")}
                ...
                <div className="overflow-x-auto hidden sm:block">
                    <table className="min-w-full text-left text-sm">
                        <thead className="text-gray-500 font-rubik uppercase border-b">
                            <tr>
                                <th className="py-2 w-[10%]">media_url</th>
                        ...
                        <tbody>
                            {loading ? (
                                ...
                                FeaturedCategoryList.map((featuredCategory) => (
                                    <tr key={featuredCategory.category.id} className="border-b hover:bg-gray-50">
                                        <td className="py-4">
                                            {featuredCategory.category.media_url ? (
                                                <img
                                                    src={featuredCategory.category.media_url}
                                                    alt={featuredCategory.category.title}
                                        ...
                                        <td className="py-6">
                                            <p className="font-medium text-gray-800">{featuredCategory.category.title}</p>
                                        </td>
                                        ...
                                        <td className="py-4 text-gray-700">
                                            {DateFormatter.formatDate(featuredCategory.created_at)}
                                        </td>
                                        <td className="py-4 text-gray-700">
                                            {DateFormatter.formatDate(featuredCategory.updated_at)}
                                        </td>
                                        <td className="py-4 text-right">
                                            <div className="relative inline-block">
                                                <button
                                                    onClick={() => toggleMenu(featuredCategory.id)}
                                                    className="text-gray-600 hover:text-black menu-toggle"
                                                >
                                                    <BsThreeDotsVertical size={18} />
                                                </button>
                                                {activeMenu === featuredCategory.id && (
                                                    <div className="dropdown-menu absolute right-0 -top-[4rem] z-10 bg-white border rounded shadow w-32">
                                                        <button
                                                            onClick={() => navigate(`/featured-categories/view/${featuredCategory.id}`)}
                                                        ...
                                                        <button
                                                            onClick={() =>
                                                                navigate(`/featured-categories/update/${featuredCategory.id}`)
                                                            }
                                                            className="flex items-center gap-2 px-3 py-2 w-full hover:bg-gray-100 text-sm text-gray-700"
                                                        >
                ...
                <div className="block sm:hidden space-y-5 flex-col items-center justify-center">
                    {!loading && FeaturedCategoryList && FeaturedCategoryList.length > 0 ? (
                            ...
                            >
                                ...
                                {activeMenu === category.id && (
                                    <div className="mt-3 dropdown-menu bg-white border rounded shadow w-full z-10">
                                        <button
                                            onClick={() => navigate(`/featured-categories/view/${category.id}`)}
                                            className="flex items-center gap-2 px-3 py-2 w-full hover:bg-gray-100 text-sm text-gray-700"
                                        >
                                            <FaEye size={14} /> View
                                        </button>
                                        <button
                                            onClick={() => navigate(`/featured-categories/update/${category.id}`)}
                                            className="flex items-center gap-2 px-3 py-2 w-full hover:bg-gray-100 text-sm text-gray-700"
                                        >
                                            <FaEdit size={14} /> Update
                                        </button>
