...
const FeaturedDestination = () => {
    ...
    const [FeaturedDestinationList, setFeaturedDestinationList] = useState([]);
    ...
    function apiCall(
        ...
        FeaturedDestinationServices.getAll(search, order, page, currentStatus)
            .then((response) => {
                setFeaturedDestinationList(response.featuredDestinations);
                setTotalPages(response.totalPages);
                setCurrentPage(response.currentPage);
                setLoading(false);
            })
            .catch((err) => {
                setFeaturedDestinationList([]);
                setTotalPages(1);
                setLoading(false);
            });
    }
        ...
        <>
            ...
            <div className="bg-white  p-4 sm:p-6 lg:ml-72 rounded-xl shadow-md font-rubik w-full max-w-6xl mx-auto -mt-2 mb-12">
                ...
                <div className="flex flex-col sm:flex-row sm:justify-between items-start sm:items-center gap-3 mb-4">
                    ...
                    {/* Add Button */}
                    <button
                        onClick={() => navigate("/featured-destinations/add")}
                ...
                <div className="overflow-x-auto hidden sm:block">
                    <table className="min-w-full text-left text-sm">
                        ...
                        <tbody>
                            ...
                            ) : FeaturedDestinationList.length > 0 ? (
                                FeaturedDestinationList.map((featuredDestination) => (
                                    <tr key={featuredDestination.destination.id} className="border-b hover:bg-gray-50">
                                        <td className="py-4">
                                            {featuredDestination.destination.media_url ? (
                                                /\.(jpeg|jpg|gif|png|webp)$/i.test(
                                                    featuredDestination?.destination?.media_url??''
                                                ) ? (
                                                    <img
                                                        src={featuredDestination?.destination?.media_url??''}
                                                        alt={featuredDestination?.destination?.title}
                                        ...
                                        <td className="py-6">
                                            <p className="font-medium text-gray-800">{featuredDestination.destination.title}</p>
                                        </td>

                                        <td className="py-6">
                                            <p className="font-medium ml-3  text-gray-800">
                                                {featuredDestination.order}
                                            </p>
                                        </td>
                                        ...
                                        <td className="py-4 text-gray-700">
                                            {DateFormatter.formatDate(featuredDestination.created_at)}
                                        </td>
                                        <td className="py-4 text-gray-700">
                                            {DateFormatter.formatDate(featuredDestination.updated_at)}
                                        </td>
                                        <td className="py-4 text-right">
                                            <div className="relative inline-block">
                                                <button
                                                    onClick={() => toggleMenu(featuredDestination.id)}
                                                    className="text-gray-600 hover:text-black menu-toggle"
                                                >
                                                    <BsThreeDotsVertical size={18} />
                                                </button>
                                                {activeMenu === featuredDestination.id && (
                                                    <div className="dropdown-menu absolute right-0 -top-[4rem] z-10 bg-white border rounded shadow w-32">
                                                        <button
                                                            onClick={() => navigate(`/featured-destinations/view/${featuredDestination.id}`)}
                                                        ...
                                                        <button
                                                            onClick={() =>
                                                                navigate(`/featured-destinations/update/${featuredDestination.id}`)
                                                            }
                                                            className="flex items-center gap-2 px-3 py-2 w-full hover:bg-gray-100 text-sm text-gray-700"
                                                        >
                ...
                <div className="block sm:hidden space-y-5 flex-col items-center justify-center">
                    {!loading && FeaturedDestinationList && FeaturedDestinationList.length > 0 ? (
                        FeaturedDestinationList.map((featuredDestination) => (
                            <div
                                key={featuredDestination.destination.id}
                                className="bg-white px-4 py-5 rounded-xl shadow-md border w-full"
                                data-aos="fade-up"
                            >
                                {featuredDestination.destination.media_url ? (
                                    /\.(jpeg|jpg|gif|png|webp)$/i.test(featuredDestination.destination.media_url) ? (
                                        <img
                                            src={featuredDestination.destination.media_url}
                                            alt={featuredDestination.destination.title}
                                            className="w-full h-48 object-cover rounded-md mb-4"
                                        />
                                    ) : /\.(mp4|webm|ogg)$/i.test(featuredDestination.destination.media_url[0].media) ? (
                                        <video
                                            src={featuredDestination.destination.media_url}
                                            controls
                                            className="w-full h-48 object-cover rounded-md mb-4"
                                ...
                                <div className="flex justify-between items-start mb-2">
                                    <div>
                                        <div className="flex items-center mb-2">
                                            ...
                                            <p className="text-sm text-gray-800">
                                                {featuredDestination.destination.title}
                                            </p>
                                        </div>
                                        ...
                                        <div className="flex items-center mb-2">
                                            ...
                                            <p className="text-sm text-gray-800">
                                                {featuredDestination.destination.description}
                                            </p>
                                        </div>
                                        ...
                                        <div className="flex items-center">
                                            ...
                                            <p
                                                className={`text-xs inline-block px-2 py-1 rounded-full font-medium ml-2 ${getStatusClass(
                                                    featuredDestination.destination.status
                                                )}`}
                                            >
                                                {featuredDestination.destination.status}
                                            </p>
                                    ...
                                    <button
                                        onClick={() => toggleMenu(featuredDestination.destination.id)}
                                ...
                                <div className="text-sm text-gray-700 space-y-2 mt-2">
                                    <p>
                                        ...
                                        {DateFormatter.formatDate(featuredDestination.destination.created_at)}
                                    </p>
                                    <p>
                                        <span className="font-bold">Updated At:</span>{" "}
                                        {DateFormatter.formatDate(featuredDestination.destination.updated_at)}
                                    </p>
                                </div>

                                {activeMenu === featuredDestination.destination.id && (
