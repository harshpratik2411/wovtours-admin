...
const stats = [
  ...
  { title: 'TODAY TRANSACTIONS', amount: '$525', change: '-0.89%', graph: 'down' },
  ...
  useEffect(() => {
    ...
    const updateChartSize = () => {
      if (window.innerWidth < 768) {
        ...
        setChartSize({ outerRadius: 80, innerRadius: 50 });
      }
    };
    ...
    <div className="p-2 space-y-8 sm:ml-0 md:ml-[155px]">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-4 w-full md:w-[71.5rem] gap-6" data-aos="fade-up">
        {stats.map((stat, idx) => {
          ...
          return (
            <div key={idx} className="bg-white p-6 rounded-xl shadow-sm space-y-3">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 w-full md:w-[71.5rem] gap-6" data-aos="fade-up">
        {/* Line Chart */}
        <div className="bg-white p-4 rounded-xl shadow-sm">
          ...
          <ResponsiveContainer width="100%" height={250}>
            <LineChart data={lineData}>
              ...
              <YAxis />
              <Tooltip />
              <Line type="monotone" dataKey="value" stroke="#ff7f50" strokeWidth={3} dot />
            </LineChart>
          </ResponsiveContainer>
        </div>
        ...
        <div className="bg-white p-4 rounded-xl shadow-sm">
          ...
          <div className="flex flex-col md:flex-row justify-between items-center w-full">
            ...
            <div className="space-y-2 mt-4 md:mt-0 md:ml-4">
              {[
                { name: 'Clothing', percent: 25, amount: '$3,020', count: 1348 },
                { name: 'Lingerie & Nightwear', percent: 35, amount: '$2,280', count: 3459 },
                { name: 'Body Fit', percent: 35, amount: '$2,280', count: 3459 },
                { name: 'Sportswear', percent: 23, amount: '$2,820', count: 879 },
                { name: 'Accessories', percent: 18, amount: '$1,224', count: 348 }
              ].map((item, idx) => (
                <div key={idx} className="text-sm text-gray-700">
                  <span className="font-medium text-black">{item.name}</span> ({item.percent}%)
                  <div className="text-xs text-gray-500">
                    {item.count} CATEGORY PRODUCTS — <span className="text-black">{item.amount}</span>
                  </div>
                </div>
              ))}
            </div>
